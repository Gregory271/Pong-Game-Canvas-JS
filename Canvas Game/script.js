...
class gameComponent {
    ...
    checkCollideWith(paddleLeft,paddleRight,puck) {
        let bottomPaddleCollide = false;
        let topPaddleCollide = false;
        let rightWallCollide = false;
        let leftWallCollide = false;
        if (((this.x + puck.width > myGameArea.canvas.width - 10) || (this.x < 10)) && ((puck.y <= this.y) && (puck.y >= this.y + 40))) { //check collide with paddle
            //sprite.x = sprite.x-1;
            topPaddleCollide = true;
            puck.speed = -puck.speed;
            console.log("puck hit the right paddle");
        } else if (((this.x + this.width > myGameArea.canvas.width - 10) || (this.x < 10))) { // check collide with wall
            rightWallCollide = true;
            console.log("puck hit the right wall");
            //gameOverScreen.style.display = "block";
            //return "Game Over!";
        }
    }
...
function updateGameArea() {
    ...
    myGameArea.clear();
    mySquare.x += mySquare.speed;
    leftPaddle.checkCollideWith(mySquare);
    rightPaddle.checkCollideWith(mySquare);
